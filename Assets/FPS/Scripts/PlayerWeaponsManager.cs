...
public class PlayerWeaponsManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (activeWeapon && m_WeaponSwitchState == WeaponSwitchState.Up)
        {
            ...
            if (activeWeapon.weaponName.Equals("Grenade"))
            {
                hasFired = activeWeapon.HandleShootInputs(Input.GetKeyDown(KeyCode.Q), Input.GetKey(KeyCode.Q), Input.GetKeyUp(KeyCode.Q));
            }
            else
            {
                // handle shooting
        ...
        if (Physics.Raycast(weaponCamera.transform.position, weaponCamera.transform.forward, out RaycastHit hit, 1000, -1, QueryTriggerInteraction.Ignore))
        {
            ...
            if (hit.collider.GetComponent<WeaponController>())
            {
                ...
                {
                    AddWeapon(hit.collider.GetComponent<WeaponController>());
                }
            }
        }
    }
